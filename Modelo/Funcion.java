    ...
    public  boolean[][] reservarAsiento(boolean[][] matriz, int columna, char fila) {
        if (columna >= 1 && columna <= matriz.length && fila >= 'A' && fila < 'A' + matriz[0].length) {
            ...
            if (matriz[filaIndex][columnaIndex] != true) {
                ...
                System.out.println("Este asiento ya está ocupado. Por favor, selecciona otro.");
            }
        } else {
            System.out.println("El parámetro ingresado es incorrecto. Por favor, ingresa una ubicación válida.");
        }
        return matriz;
    }
